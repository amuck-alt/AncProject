...
<div class="form-group">
	<label>선택항목 </label>
		<div class="form-check">
			...
			<input type="checkbox"
			<c:if test="${fn:indexOf(result.hopse, '항공운항과 대학입시') != -1 }">checked</c:if>
			value="항공운항과 대학입시" name="hopse" id="hopke" class="form-check-input">항공운항과 대학입시
			</label>
		</div>
		<div class="form-check">	
			<label class="form-check-label">
         <input type="checkbox"
         <c:if test="${fn:indexOf(result.hopse, '항공경영과 대학입시') != -1 }">checked</c:if>
         value="항공경영과 대학입시" name="hopse" id="hopko" class="form-radio-input">항공경영과 대학입시
         </label>
		</div>
		<div class="form-check">	
			<label class="form-check-label">
         <input type="checkbox"
         <c:if test="${fn:indexOf(result.hopse, '호텔경영과 대학입시') != -1 }">checked</c:if>
         value="호텔경영과 대학입시" name="hopse" id="hopos" class="form-radio-input">호텔경영과 대학입시
         </label>
		</div>
</div>
...
<script>
	...
	function deleteRow(idx){
		if(confirm("정말로 삭제하시겠습니까?") == true){
			location.href="/manager/counsel/delupdate.do?reurl=/manager/spclass.do&idx="+idx;	
		}else{
			return;
		}		
	}
